...
// Import via external API
router.post('/import', verify, async (req, res, next) => {
  try {
    const p = await importRandomImage();
    const rec = await recordImported(req.user.username, p);
    res.status(201).json({ id: rec.id, meta: rec.meta, size: rec.size, format: rec.format });
  } catch (e) { next(e); }
});
...
router.get('/:id/download', verify, async (req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT original_path, owner_username FROM images WHERE id=$1`, [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'not_found' });
    const row = rows[0];
    if (req.user.role !== 'admin' && row.owner_username !== req.user.username) {
      return res.status(403).json({ error: 'forbidden' });
